...
public class SaveGameManager
{
	...
	public void Save(string saveGameName, string newLevelName)
	{
		...
		CurrentSave.LevelSpawnPointName = GameManager.Inst.PlayerProgress.LevelSpawnPointName;
	...
	public bool Load(string saveGameName)
	{
		...
		{
			Debug.Log("checking levels " + level.Name);
		...
		//place player in last saved location or if there's level spawn point name, place player there
		if(!String.IsNullOrEmpty(CurrentSave.LevelSpawnPointName))
		{
			GameObject go = GameObject.Find(CurrentSave.LevelSpawnPointName);
			if(go != null)
			{
				LevelPortal portal = go.GetComponent<LevelPortal>();
				GameManager.Inst.PlayerControl.SelectedPC.transform.position = portal.SpawnPoint.position;
				//advance time
				float hours = GameManager.Inst.WorldManager.GetTravellingTime(portal.TravelDistance * 1f);
				GameManager.Inst.WorldManager.AdvanceTime(Mathf.FloorToInt(hours), Mathf.FloorToInt((hours - Mathf.FloorToInt(hours)) * 60));
				GameManager.Inst.WorldManager.ChangeEnvironment(CurrentSave.CurrentEnvironmentName);
			}
		}
		else
