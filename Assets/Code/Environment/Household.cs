...
public class Household : MonoBehaviour
{
	...
	private void RefillHouseHoldSquadMembers()
	{
		...
		if(CurrentSquad.Members.Count <= 0 && !CurrentSquad.CanRespawn)
		{
			return;
		}
		...
		if(GameManager.Inst.NPCManager.GetFactionData(CurrentSquad.Faction).CharacterType == CharacterType.Mutant)
		{
			...
			{
				int humansCount = GameManager.Inst.NPCManager.HumansInScene.Count;
				string speakerName = GameManager.Inst.NPCManager.HumansInScene[UnityEngine.Random.Range(1, humansCount)].Name;
				GameManager.Inst.UIManager.SetAnnouncementText(speakerName + " : Attention! Ghouls spotted!");
			}
		}
